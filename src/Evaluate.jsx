...
const ModelComparisonLab = ({ aiSettings, assessments, submissions }) => {
  ...
  const exportPython = () => {
    ...
    rRawQuestions.forEach((qObj, idx) => {
      ...
      aiModels.forEach(m => {
        const aiScoreObj = m.individualScores[idx];
        row[`${m.model} Score`] = aiScoreObj ? aiScoreObj.score : null;
      });
      data.push(row);
    });
...
# Auto-generated by GRADER.ai
# AI Model Comparison Data
data_json = """
${JSON.stringify(data, null, 2)}
"""

print("Loading GRADER.ai Comparison Data...")
...
`;
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'grader_ai_analysis_' + Date.now() + '.py'; a.click();
    URL.revokeObjectURL(url);
  };
        ...
        <div style={{ display:'flex', gap:'12px', flexWrap:'wrap', alignItems: 'stretch' }}>
          ...
          {rResults.length > 0 && (
            <div style={{ display: 'flex', gap: '8px' }}>
              ...
              <button className="btn" onClick={exportPython} style={{ padding: '8px 16px', background: 'var(--panel-border)', border: '1px solid var(--panel-border)' }}><Terminal size={16} /> Generate Python Script</button>
            </div>
          )}
        </div>
    ...
    if (studentTabState === 'results') {
      ...
      if (hasPending) {
        interval = setInterval(async () => {
          try {
            const { data: subRes, error } = await supabase.from('submissions').select('*').order('created_at', { ascending: false }).limit(50);
            if (subRes) {
                ...
                <div style={{ display: 'flex', flexDirection: 'column', flex: 2 }}>
                  ...
                  {aiGenerating && (
                    <div style={{ width: '100%', height: '2px', background: 'rgba(59,130,246,0.2)', marginTop: '6px', overflow: 'hidden', borderRadius: '2px', position: 'relative' }}>
                      <div style={{ position: 'absolute', top: 0, left: 0, height: '100%', width: '40%', background: '#3b82f6', borderRadius: '2px', animation: 'shimmer 1.5s infinite linear reverse' }}></div>
                    </div>
                  )}
                </div>
                {editingAssessmentId && (
          ...
          try {
            ...
            if (!shouldQueue) {
              ...
              try {
                ...
                if (window.showToast) window.showToast("High traffic detected. Your exam has been queued for background grading.", "info");
                console.warn(`Live grading failed due to high load: ${e.message}. Falling back to background queue.`);
                // We leave status as 'pending' and results as null. The lecturer dashboard will grade it later.
                newSub.status = 'pending';
                newSub.results = null;
                newSub.authenticity = null;
              }
            }
