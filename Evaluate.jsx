...
export default function EvaluateApp() {
  ...
  const markSubmission = async (assessment, answers, studentFiles = []) => {
    const system = "Expert Academic Grader. Grade strictly according to the Reference Context provided. IF AN ANSWER IS INCOHERENT, FACTUALLY WRONG, OR COMPLETELY IRRELEVANT, AWARD EXACTLY 0 POINTS. DO NOT GIVE POINTS FOR EFFORT. Keep feedback EXTREMELY BRIEF (under 15 words) to maximize generation speed. Return a RAW JSON object with EXACTLY this structure: {\"results\": [{\"questionId\": <number>, \"score\": <number>, \"grade\": \"<string>\", \"feedback\": \"<short string>\", \"strengths\": [\"<1 word>\"], \"improvements\": [\"<1 word>\"]}], \"authenticity\": <number 0-100>, \"authenticityReason\": \"<short string>\"}.";
    const prompt = `Grading task for: ${assessment.title}\nQuestions: ${JSON.stringify(assessment.questions)}\nStudent Typed Answers: ${JSON.stringify(answers)}\nReference Context: ${assessment.contextText || courseMaterial.text}\nIf a student file is attached, read the answers directly from the file to grade. Strictly evaluate for AI-generation or plagiarism. Keep responses ultra-concise for speed.`;
