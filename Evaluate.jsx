  ...
  React.useEffect(() => {
    ...
    class Particle {
      ...
      draw() {
        ...
        ctx.fillStyle = 'rgba(201, 209, 217, 0.4)'; // github text-main color
        ctx.fill();
      }
    }
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distance < 120) {
            ctx.beginPath();
            ctx.strokeStyle = `rgba(201, 209, 217, ${0.15 - distance / 120 * 0.15})`;
            ctx.lineWidth = 0.6;
...
// ─── Route Auth Screens ───────────────────────────────────────────────────
  if (!role) {
    if (authScreen === 'student-login') return <><GlobalStyles /><ParticleBackground /><div className="blueprint-grid" /><FlyingTelegramLogo /><TerminalBackground /><StudentLoginScreen /></>;
    return <><GlobalStyles /><ParticleBackground /><div className="blueprint-grid" /><FlyingTelegramLogo /><TerminalBackground /><LoginScreen />{loginModalRole && RoleLoginModal()}</>;
  }

  if (role === 'FacultyHub') {
    return <><GlobalStyles /><ParticleBackground /><div className="blueprint-grid" /><FlyingTelegramLogo /><TerminalBackground /><FacultyHubScreen /></>;
  }

  return (
    <>
      <GlobalStyles /><ParticleBackground /><div className="blueprint-grid" /><FlyingTelegramLogo /><TerminalBackground />
